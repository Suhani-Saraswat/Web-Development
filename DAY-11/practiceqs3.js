/* Create a system to calculate popcorn prices based on the size customer asked for:
if size is 'XL', price is Rs. 250
if size is 'L', price is Rs. 200
if size is 'M', price is Rs. 100
if size is 'S', price is Rs. 50 */

let size = "L";

if(size === "XL"){
    console.log(`Size is ${size}, so Price is Rs.250`);
}else if(size === "L"){
    console.log(`Size is ${size}, so Price is Rs.200`);
}else if(size === "M"){
    console.log(`Size is ${size}, so Price is Rs.100`);
}else{
    console.log(`Size is ${size}, so Price is Rs.50`);
}